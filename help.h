...
//--------------- For Making color filled ellipse---------------//
void myFilledEllipse(int x1, int y1, int x2, int y2, int R, int G, int B, int backR, int backG, int backB)
{
	HWND console_handle = GetConsoleWindow();
    HDC device_context = GetDC(console_handle);
    ...
    HPEN pen =CreatePen(PS_SOLID,2,RGB(R,G,B)); 
    SelectObject(device_context,pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(backR,backG,backB));  // back means background fill color.
	SelectObject(device_context,brush);
	Ellipse(device_context,x1,y1,x2,y2);
	ReleaseDC(console_handle, device_context);
	DeleteObject(pen);
